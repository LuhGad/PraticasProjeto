...
public class ReceitasDao {
  private Connection conn;
	...
	public void updateReceita(Receita receita){
		String sql = "UPDATE RECEITAS SET ingredientes=?, preparo=?, tempo=?, rendimento=?, nome=?  where id=?";
	...
	public Receita resourceReceita (int id) throws SQLException {
        ...
        PreparedStatement pst = conn.prepareStatement("SELECT * FROM RECEITAS WHERE ID = ?");
        pst.setInt(1, id);
        pst.execute();
	...
	public Receita recuperaReceita(String nome) {
		...
		try {
			...
			pst = conn.prepareStatement("SELECT * FROM RECEITAS WHERE NOME = ?");
